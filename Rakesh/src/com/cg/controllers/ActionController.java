...
public ModelAndView getUpdate(@ModelAttribute("trainee") Trainee trainee) {
	...
	public ModelAndView login(@RequestParam("username") String username, @RequestParam("password") String password, @ModelAttribute("trainee") Trainee trainee) {
				boolean flag = service.validate(username,password);
				if(flag==true) {
				ModelAndView modelAndView = new ModelAndView("indexListPage", "trainee", trainee);
				return modelAndView;
				}
				else
				{
					ModelAndView modelAndView = new ModelAndView("viewDispPage");
					return modelAndView;
				}
		}
